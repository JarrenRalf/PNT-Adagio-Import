...
/**
 * This function runs when the compute conversions button is clicked on the ProposedConvertedExport page.
 * 
 * @author Jarren Ralf
 */
...
function computeConversions_Assembly()
{
  ...
  try
  {
    ...
    const   exportSheet = spreadsheet.getSheetByName("ProposedConvertedExport");
...
function resetData()
{
  ...
  try
  {
    ...
    spreadsheet.getSheetByName("ProposedConvertedExport").getRange(1, 3, 2).setValues([[runtime], [timeStamp(spreadsheet)]]); // Elapsed time and timestamp
...
/**
* This function calls the resetData function and passes the function a true value which ensure that a timestamp is placed on the ProposedConvertedExport page
*
* @author Jarren Ralf
*/
function resetData_ButtonClicked_On_ProposedConvertedExport()
{
