...
function applyFullRowFormatting(sheet, row, numRows, numCols)
{
  ...
  if (SHEET_NAME === "Manual Counts" || SHEET_NAME === "InfoCounts")
  {
    var numberFormats = [...Array(numRows)].map(e => ['@', '#.#', '0.#', ...new Array(numCols - 3).fill('@')]);
  ...
  else if (SHEET_NAME === "Tites Counts")
  {
...
function copySelectedValues(sheet, startRow, numCols, qtyCol, isInfoCountsPage, sheets, isNotFound)
{
  ...
  {   
    ...
    {
      const nCols = (sheet.getSheetName() === 'Manual Counts') ? 4 : 11;
...
{
  var html = HtmlService.createHtmlOutputFromFile('DownloadButton').setWidth(250).setHeight(50)
 ...
 * @author Jarren Ralf
 */
function downloardInflowPickList()
{
  const sheet = ss.getSheetByName('inFlowPick');
  const data = sheet.getSheetValues(2, 1, sheet.getLastRow() - 1, sheet.getLastColumn() - 1)
...
function manualScan(e, spreadsheet, sheet)
{
  ...
  else if (barcodeInputRange.columnStart !== 1) // Quantity is entered
  {
    ...
    if (isNotBlank(quantity)) // The user may have hit the delete key
    {
      ...
      else if (quantity.toString().split(' ', 1)[0].toLowerCase() === 'uuu') // Unmarry upc
      {
      ...
      else if (quantity.toString().split(' ', 1)[0].toLowerCase() === 'mmm') // Marry upc
      {
        const upc = quantity.split(' ')[1];
...
function onDemand()
{
  ...
  ss.getSheetByName('On Demand Items').clearContents().getRange(1, 1, numRows, 12).setNumberFormat('@').setValues(onDemand)
}
