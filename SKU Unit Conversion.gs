...
{
  SpreadsheetApp.getActiveSheet().getRange(2, 9).isChecked() ? computeConversions_Yeti() : computeConversions_Assembly_Then_PackageSize();
}
...
function computeConversions_Assembly()
{
  ...
  try
  {
        ...
        for (var l = 0; l < numLocations; l++)
        {
          if (assembledSkusAndQtys[assemblyData[i][ASSEMBLED_SKU].toString().toUpperCase()][l] < 0) // Need to compute an assembly
          {
            for (var k = 1; k < numItems; k++)
            {
              ...
              {
                itemValues = csvData.find(sku => sku[8].toString().toUpperCase() === data[k][SKU].toString().toUpperCase()) // Find the item in the testInventory csv
                ...
                exportData[l].push([data[k][SKU], (itemValues) ? itemValues[9] : '', data[k][locations[l]]])
                break;
              }
            }
              ...
              if (data[j][locations[l]] < 0) // Need to compute an assembly
              {
                for (var k = 1; k < numItems; k++)
                {
                  if (data[k][SKU] == assemblyData[i][COMPONENT_SKU])
                  {
                    ...
                    data[k][locations[l]] = Number(data[k][locations[l]])
                    data[k][locations[l]] += assembledSkusAndQtys[assemblyData[i][ASSEMBLED_SKU].toString().toUpperCase()][l]*Number(assemblyData[i][CONVERSION_FACTOR])
                    exportData[l].push([data[j][SKU], (itemValues) ? itemValues[9] : '', 0], [data[k][SKU], (itemValues) ? itemValues[9] : '', data[k][locations[l]]])
                    break;
                  }
                }
    ...
    exportSheet.getRange(8, 2, exportSheet.getLastRow(), 11).clearContent();
    exportData.map((loc, i) => {if (loc.length !== 0) exportSheet.getRange(8, 2 + 4*i, loc.length, 3).setValues(loc);})
    
    const headerRange = exportSheet.getRange(1, 1, 2, 10);
    const headerValues = headerRange.getValues();
    headerValues[0][0] = "Function Run Time";
    headerValues[0][5] = "Conversion Export";
    headerValues[1][9] = timeStamp(spreadsheet);
  ...
  try
  {
    ...
    for (var j = 2; j < numConversions; j++)
    {
      ...
      for (var i = 1; i < numItems; i++)
      {
        ...
        {
          [data, exportData] = getConversions(data, exportData, conversionData[j], pairOfSKUs, SKU, CONVERSION_FACTOR, locations, csvData);
  ...
  try
  {
    ...
    for (var j = 2; j < numConversions; j++)
    {
      ...
      for (var i = 1; i < numItems; i++)
      {
        ...
        if (bothSKUsAreFound(pairOfSKUs)) // Once both of the SKUs are found, then get the conversions for all four locations
        {
          [data, exportData] = getConversions_Yeti(data, exportData, pairOfSKUs, SKU, locations, csvData);
          pairOfSKUs.length = 0; // Empty the array by setting its length equal to 0
          break;
        }
      }
    ...
    exportData.map((loc_, i) => {
      ...
      if (loc.length !== 0) exportSheet.getRange(8, 2 + 4*i, loc.length, 3).setValues(loc);
    })
...
* @param {Object[][]}        csvData : The adagio data with the included cost field
...
* @author Jarren Ralf
*/
function getConversions(data, exportData, conversionData, pairOfSKUs, SKU, CONVERSION_FACTOR, locations, csvData)
{
    ...
    if (!isNonPositive(data[pairOfSKUs[SMALLER_PACK]][locations[l]]) || !isNonPositive(data[pairOfSKUs[LARGER_PACK]][locations[l]])) // If both items have a non-positive stock, don't enter the loop
    {
      ...
      {  
        ...
        if (numPacksNeeded < data[pairOfSKUs[LARGER_PACK]][locations[l]]) // Compute the relevant conversions
        {
          ...
          exportData[l].push([data[pairOfSKUs[LARGER_PACK]][SKU], (itemValues1) ? itemValues1[9] : '', data[pairOfSKUs[LARGER_PACK]][locations[l]]]);
          data[pairOfSKUs[SMALLER_PACK]][locations[l]] += numPacksNeeded*conversionData[CONVERSION_FACTOR];
          exportData[l].push([data[pairOfSKUs[SMALLER_PACK]][SKU], (itemValues2) ? itemValues2[9] : '', data[pairOfSKUs[SMALLER_PACK]][locations[l]]]);
        }
...
* @author Jarren Ralf
*/
function getConversions_Yeti(data, exportData, pairOfSKUs, SKU, locations, csvData)
{
  ...
  for (l = 0; l < numLocations; l++)
  {
    ...
    {
      itemValues1 = csvData.find(sku => sku[8].toString().toUpperCase() === data[pairOfSKUs[ACTIVE]][SKU].toString().toUpperCase()) // Find the item in the testInventory csv
      itemValues2 = csvData.find(sku => sku[8].toString().toUpperCase() === data[pairOfSKUs[DISCONT]][SKU].toString().toUpperCase()) // Find the item in the testInventory csv
      ...
      exportData[l].push([data[pairOfSKUs[ACTIVE]][SKU], (itemValues1) ? itemValues1[9] : '', data[pairOfSKUs[ACTIVE]][locations[l]]]);
      exportData[l].push([data[pairOfSKUs[DISCONT]][SKU], (itemValues2) ? itemValues2[9] : '', data[pairOfSKUs[DISCONT]][locations[l]]]);
    }
  }
...
function resetData()
{
  ...
  try
  {
    ...
    const timeStampRng = spreadsheet.getSheetByName("Adagio Transfer Sheet").getRange(1, 5, 2, 7);
    const timeStampValues = timeStampRng.getValues();
    timeStampValues[0][6] = runtime + ' seconds';
