...
{
  SpreadsheetApp.getActiveSheet().getRange(2, 7).isChecked() ? computeConversions_Yeti() : computeConversions_PackageSize();
}
...
{
  const START_TIME = new Date().getTime(); // To calculate the elapsed time
  const SEASONAL_SKU  = 0;                 // For the jth row of the SKUsToWatch, the SKU on the LEFT  (Used as an index for the conversionData array)
  const DISCONT_SKU  = 5;                  // For the jth row of the SKUsToWatch, the SKU on the RIGHT (Used as an index for the conversionData array)
